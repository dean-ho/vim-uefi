...
" }}}
syn match   aslCommentSkip        contained "^\s*\*\($\|\s\+\)"
syn region  aslCommentL          start="//" skip="\\$" end="$" keepend contains=aslComment2String,@Spell
syn region  aslComment           start="/\*" end="\*/" extend contains=aslComment2String,@Spell
syn region  aslCommentString     contained start=+L\=\\\@<!"+ skip=+\\\\\|\\"+ end=+"+ end=+\*/+me=s-1 contains=aslSpecial,aslCommentSkip
syn region  aslComment2String    contained start=+L\=\\\@<!"+ skip=+\\\\\|\\"+ end=+"+ end="$" contains=aslSpecial
syn region  aslString            start=+\(L\|u\|u8\|U\|R\|LR\|u8R\|uR\|UR\)\="+ skip=+\\\\\|\\"+ end=+"+
" Highlight special characters (those which have a backslash) differently
syn match   aslSpecial            display contained "\\\(x\x\+\|\o\{1,3}\|.\|$\)"
syn match   aslName               "\<\(\u\|_\)\(\u\|\d\|_\)\{3\}\>"
syn match   aslNumbers            display transparent "\<\d\|\.\d" contains=aslNumber
syn match   aslNumber             "0x\x\+\(u\=l\{0,2}\|ll\=u\)\>"
syn match   aslNumber             display contained "\d\+\(u\=l\{0,2}\|ll\=u\)\>"

syn cluster aslParenGroup       contains=aslIncluded,aslSpecial,aslCommentSkip,aslNumber
syn region  aslParen             transparent start='(' end=')' contains=ALLBUT,@aslParenGroup,@Spell
syn region  aslPreCondit         start="^\s*\zs\(%:\|#\)\s*\(if\|ifdef\|ifndef\|elif\)\>" skip="\\$" end="$" keepend contains=aslComment,aslCommentL,aslString
syn match   aslPreConditMatch     display "^\s*\zs\(%:\|#\)\s*\(else\|endif\)\>"
syn region  aslIncluded          display contained start=+"+ skip=+\\\\\|\\"+ end=+"+
syn match   aslIncluded           display contained "<[^>]*>"
syn match   aslInclude            display "^\s*\zs\(%:\|#\)\s*include\>\s*["<]" contains=aslIncluded
syn cluster aslPreProcGroup     contains=aslPreCondit,aslIncluded,aslInclude,aslDefine,aslSpecial,aslNumber,aslString,aslCommentSkip,aslParen
syn region  aslDefine            start="^\s*\zs\(%:\|#\)\s*\(define\|undef\)\>" skip="\\$" end="$" keepend contains=ALLBUT,@aslPreProcGroup,@Spell
syn region  aslPreProc           start="^\s*\zs\(%:\|#\)\s*\(pragma\>\|line\>\|warning\>\|warn\>\|error\>\)" skip="\\$" end="$" keepend contains=ALLBUT,@aslPreProcGroup,@Spell
syn match   aslInvocation         "\<\([A-Z0-9_]\{4}\|[A-Z0-9]\{3}\)\{1}\s*(\@=" contains=cParen
syn region  aslMethodObj         start="\(\<Method\s*(\s*\)\@<=\([A-Z0-9_]\{3,4}\)\{1}" end="\(\s*\(,\|)\)\)\@=" oneline
...
hi def link aslNumbers          Number
hi def link aslNumber           Number
hi def link aslString           String
hi def link aslCommentString    aslString
hi def link aslComment2String   aslString
hi def link aslIncluded         aslString
" Identifier - Function
hi def link aslInvocation       Function
hi def link aslMethodObj        Function
...
hi def link aslDefine           Macro
hi def link aslPreConditMatch   aslPreCondit
hi def link aslPreCondit        PreCondit
" Type - StorageClass Structure Typedef
" Special - SpecialChar Tag Delimiter SpecialComment Debug
hi def link aslSpecial          SpecialChar
" Underlined
" Ignore
" Error
" Todo
